...
#artistas
    ...
    for line in lista:
        ...
        artistas +=[artista]
    for j in range(len(artistas)):
        idMusica = artistas[j]['IdMusicas']
        id_split = []
        id_split = artistas[j]['IdMusicas'].split(',')  #campo de artistas idMusica sendo tranformado em lista de ids de musicas do artista desejado
        for i in range(len(id_split)):
            if(IdMusicaEscolhida == id_split[i]):
                print('Artista é', artistas[j]['Nome'])
...
def pesquisaArtista():
    ...
    Digito = input()
    #tratamento do erro : caso o usuario escreva letras
    valor = trataerro(Digito)
    if(valor == -2):
        print('operacao incorreta, volte ao menu')
        sleep(1)
...
def trataerro(Digito):
    ...
    if(len(Digito) == 1):
        for index in range (len(alfabeto)) :
            if alfabeto[index] ==  Digito:
                valor = -2
